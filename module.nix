...
let
  inherit (lib)
    boolToString escapeShellArg mkAfter mkDefault mkEnableOption mkForce mkIf
    mkOption optionalAttrs optionalString types;
in {
  ...
  config = let
    inherit (import ./constants.nix)
      name linuxHostName linuxUser sshKeyType sshHostPrivateKeyFileName
      sshHostPublicKeyFileName sshUserPrivateKeyFileName
    ...
    darwinGroup = builtins.replaceStrings [ "-" ] [ "" ]
      name; # keep in sync with `name`s format
    darwinUser = "_${darwinGroup}";
    linuxSshdKeysDirName = "linux-sshd-keys";

    sshGlobalKnownHostsFileName = "ssh_known_hosts";
    sshHost =
      name; # no prefix because it's user visible (in `sudo ssh '${sshHost}'`)
    ...
    launchd.daemons."${daemonName}" = {
      ...
      script = let
        ...
        sshGlobalKnownHostsFileNameSh =
          escapeShellArg sshGlobalKnownHostsFileName;
      ...
      in ''
        ...
        find ${sshUserPrivateKeyFileNameSh} \
          -perm '-go=r' -exec ${
            boolToString cfg.permitNonRootSshAccess
          } '{}' '+' 2>'/dev/null' && \
        true || {
        ...
        exec limactl start ${
          optionalString debugInsecurely "--debug"
        } --foreground ${vmNameSh}
      '';
